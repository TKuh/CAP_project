...
InstallGlobalFunction( "CAP_JIT_INTERNAL_GET_OUTPUT_TYPE_OF_GLOBAL_FUNCTION_BY_INPUT_TYPES", function ( gvar, input_types )
  local input_filters, info, pos, category, type_signatures, output_type;
    ...
    if IsBound( CAP_INTERNAL_METHOD_NAME_RECORD.(gvar) ) then
        ...
        if IsSpecializationOfFilter( "category", input_types[1].filter ) and IsSpecializationOfFilterList( CAP_INTERNAL_REPLACED_STRINGS_WITH_FILTERS( info.filter_list, input_types[1].category ), input_filters ) then
...
CapJitAddTypeSignatureDeferred( "MatricesForHomalg", "HomalgRing", [ "IsHomalgMatrix" ], "IsHomalgRing" );
