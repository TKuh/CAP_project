...
InstallGlobalFunction( SGREPS_RightDistributivityExpandingPermutation,
  function( sgreps, L, a, L_tensor_a )
    local irreducible_characters, L_length, L_sum, L_sum_nr_support, L_sum_support, a_nr_support, a_support, a_components, L_tensor_a_nr_support, L_tensor_a_support, permutations;
        ...
        a_times_nk :=
            List( [ 1 .. L_sum_nr_support ], i ->
                List( [ 1 .. a_nr_support ], j ->
                    a_components[j] * SGREPS_ScalarProduct( irreducible_characters, L_tensor_a_support[k], L_sum_support[i], a_support[j] ) ) );
    ...
    permutations := List( [ 1 .. tensor_product_nr_support ], function( k )
        ...
        return Concatenation( List( [ 1 .. a_nr_support ], function( i )
            return Concatenation( List( [ 1 .. morphism_nr_support ], function( j )
                ...
                list := SGREPS_KroneckerProductOfPermutationWithIdentityFromLeft( sgreps, list, a_components[i] );
  ...
  function( sgreps, L, direct_sum )
    ...
    sum_permutations := List( [ 1 .. sum_nr_support ], function( k )
        ...
        rows := List( [ 1 .. L_length ], function( i )
            ...
            return [ BigInt( 0 ), Length( Concatenation( permutations{ Positions( support, sum_support[k] ) } ) ) ][ 1 + BooleanToInteger( sum_support[k] in support ) ];
        ...
        return Concatenation( List( [ 1 .. L_length ], function( i )
            ...
            permutation := [ CapJitTypedExpression( [], { } -> CapJitDataTypeOfListOf( IsBigInt ) ), Concatenation( permutations{ Positions( support, sum_support[k] ) } ) ][ 1 + BooleanToInteger( sum_support[k] in support ) ];
