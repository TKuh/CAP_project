...
InstallMethod( SkeletalCategoryOfGroupRepresentations,
  ...
  function( CAP_NAMED_ARGUMENTS, G, splitting_field )
    ...
    Coproduct := CoproductOfCategoryOfRowsWithSparseDatastructure( Rows,
    ...
    modeling_tower_object_constructor :=
      function( SGReps, list_of_pairs_of_rank_and_index )
        ...
        list_of_pairs_of_object_and_index :=
            List( [ 1 .. Length( list_of_pairs_of_rank_and_index ) ], i ->
                Pair( CategoryOfRowsObject( Rows, list_of_pairs_of_rank_and_index[i][1] ),
                      list_of_pairs_of_rank_and_index[i][2] ) );
    ...
    modeling_tower_object_datum :=
      function( SGReps, object )
        local list_of_pairs_of_object_and_index, list_of_pairs_of_rank_and_index;
        
        list_of_pairs_of_object_and_index := ListOfPairsOfObjectAndIndex( object );
        
        # Turn the objects in the category of rows into their ranks/integers.
        return List( [ 1 .. Length( list_of_pairs_of_object_and_index ) ], i ->
            Pair( RankOfObject( list_of_pairs_of_object_and_index[i][1] ),
                  list_of_pairs_of_object_and_index[i][2] ) );
      ...
      function( SGReps, source, list_of_pairs_of_matrix_and_index, target )
        ...
        list_of_pairs_of_morphism_and_index :=
            List( list_of_pairs_of_matrix_and_index, pair ->
                Pair( AsCategoryOfRowsMorphism( Rows, pair[1] ),
                      pair[2] ) );
    ...
    SGReps!.compiler_hints :=
        rec( category_attribute_names :=
            [ "UnderlyingCoproductOfCategoryOfRows",
...
InstallGlobalFunction( INSTALL_FUNCTIONS_FOR_SKELETAL_CATEGORY_OF_GROUP_REPRESENTATIONS,
  ...
  function( SGReps, irreducible_characters, nr_irreducible_characters )
    local decompose_product_of_characters;
    ...
    AddTensorUnit( SGReps,
      function( SGReps )
        ...
        return ObjectConstructor( SGReps, [ [ 1, unit_index ] ] );
    ...
    AddTensorProductOnObjects( SGReps,
      function( SGReps, object_1, object_2 )
        local Coproduct, Rows, irreducible_characters, pairs_1, pairs_2, product;
        ...
        pairs_1 := ListOfPairsOfObjectAndIndex( ModelingObject( SGReps, object_1 ) );
        pairs_2 := ListOfPairsOfObjectAndIndex( ModelingObject( SGReps, object_2 ) );
        ...
        #        = 2χ₁⊕ 8χ₂⊕ 5χ₃⊕ 9χ₄
        product :=
            DirectSum( SGReps, List( pairs_1, pair_1 ->
                DirectSum( SGReps, List( pairs_2, function( pair_2 )
                    ...
                    decomposed_product :=
                        CATEGORY_OF_SKELETAL_GROUP_REPRESENTATIONS_DECOMPOSE_CHARACTER( Rows, irreducible_characters, product );
                    
                    decomposed_product :=
                        List( decomposed_product, pair ->
                            Pair( TensorProductOnObjects( Rows, pair[1], multiplicity_of_product ),
                                  pair[2] ) );
                    
                    decomposed_product := ObjectConstructor( Coproduct, decomposed_product );
                    
                    return ReinterpretationOfObject( SGReps, decomposed_product );
    ...
    AddTensorProductOnMorphismsWithGivenTensorProducts( SGReps,
      function( SGReps, source, alpha, gamma, target )
        local Coproduct, Rows, nr_irreducible_characters, irreducible_characters, pairs_alpha, pairs_gamma, tensored_morphisms, sums_of_morphisms, morphism_pairs;
        ...
        pairs_alpha := ListOfPairsOfMorphismAndIndex( ModelingMorphism( SGReps, alpha ) );
        pairs_gamma := ListOfPairsOfMorphismAndIndex( ModelingMorphism( SGReps, gamma ) );
        
        tensored_morphisms := Cartesian( [ pairs_alpha, pairs_gamma ] );
        
        tensored_morphisms :=
            List( tensored_morphisms, pair ->
                Pair( TensorProductOnMorphisms( Rows, pair[1][1], pair[2][1] ), Pair( pair[1][2], pair[2][2] ) ) );
        
        # 'tensored_morphisms' is now a list of pairs where
        # - the first index is a tensored morphism,
        # - the second index is the pair of indices of morphisms, which were used in the tensor product.
        # I.e., it takes the form [ ..., [ ɑᵢ⊗ ɣⱼ, [i,j] ], ... ].
        
        # TODO: Do this in a non-functional way or manually compile it?
        # 
        #       1) Due to the ZeroMorpism( Rows, ... ) below we might
        #          sum over many unnecessary zero morphism 0 -> 0
        #          and might even end up with zero morphisms in
        #          the final tensor product.
        # 
        ...
        # (ɑ⊗ ɣ)ₗ := ⊕ ᵢ⊕ ⱼ (ɑᵢ⊗ ɣⱼ⊗ Iₙ₍ᵢⱼ₎ₗ)
        sums_of_morphisms :=
            List( [ 1 .. nr_irreducible_characters ], l ->
                DirectSumFunctorialWithGivenDirectSums( Rows,
                    Component( ModelingObject( SGReps, source ), l ),
                    List( tensored_morphisms,
                      function( pair )
                        local i, j, chi_i, chi_j, chi_l, product, n_ijl, alpha_x_gamma, identity_morphism;
                        
                        i := pair[2][1];
                        j := pair[2][2];
                        
                        chi_i := irreducible_characters[ i ];
                        chi_j := irreducible_characters[ j ];
                        chi_l := irreducible_characters[ l ];
                        
                        product := chi_i * chi_j;
                        
                        # Get the multiplicity of 'chi_l' in 'product'.
        ...
        morphism_pairs :=
            List( [ 1 .. nr_irreducible_characters ], i ->
                Pair( UnderlyingMatrix( sums_of_morphisms[i] ), i ) );
        
        return MorphismConstructor( SGReps, source, morphism_pairs, target );
        
    end );
    
    # AddLeftDistributivityExpanding( SGReps,
    #   function( SGReps, source, obj, list, target )
    #
    #
    #
    # end );
...
InstallGlobalFunction( CATEGORY_OF_SKELETAL_GROUP_REPRESENTATIONS_DECOMPOSE_CHARACTER,
