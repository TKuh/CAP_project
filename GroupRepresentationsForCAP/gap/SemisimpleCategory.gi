...
InstallGlobalFunction( CAP_INTERNAL_INSTALL_OPERATIONS_FOR_SEMISIMPLE_CATEGORY,
  function( category, tensor_unit, associator_data, is_complete_data )
    local underlying_category, field, membership_function, associator_on_irreducibles, braiding_on_irreducibles,
          ...
          distributivity_function, associator_available, is_magma_ring, right_distributivity_expanding_permutation_functional, left_distributivity_expanding_permutation_functional;
    ...
    right_distributivity_expanding_permutation_functional := FunctionWithCache(
      function( object_b, list_of_objects, direct_sum, support_tensor_product, is_expanded )
        local object_b_list, direct_sum_support, size_objs;
        
        size_objs := Length( list_of_objects );
        object_b_list := SemisimpleCategoryObjectList( object_b );
        direct_sum_support := Support( direct_sum );
        
        # Outer loop over 'k' (returns the list of pairs [ perm, k ])
        return List( support_tensor_product, function( k )
            local k_permutation;
            
            k_permutation := Concatenation( List( [ 1 .. size_objs ], function( l )
                local height, current_obj;
                
                height := 0;
                current_obj := list_of_objects[l];
                
                return Concatenation( List( direct_sum_support, function( i )
                    local multiplicity_li, sum_up_to_l_minus_1, multiplicity_directsum_i;
                        
                    multiplicity_li := Multiplicity( current_obj, i );
                    ...
                    return Concatenation( List( object_b_list, function( j )
                        local b_j_times_c_kij, cols, rows, height_of_zeros, previous_height, result;
                        ...
                        result := List( [ 1 .. cols ], m -> previous_height + height_of_zeros + m );
    ...
    associator_on_irreducibles := function( object_1, object_2, object_3 )
      ...
      if is_complete_data then
          ...
          # Display( object_2 );
          # Display( object_3 );
          # Error( "\033[31mDEBUGPRINT[irreducible]\033[0m" );
    ...
    AddAssociatorLeftToRightWithGivenTensorProducts( category,
      function( cat, new_source, object_a, object_b, object_c, new_range )
        local object_a_list, object_b_list, object_c_list, result_morphis,
              ...
              tensor_product, first_permutation,
              second_permutation, chi,
              perm, perm1, perm2, perm3, dim, vector_space_object, homalg_matrix, support,
              ...
              morphism_4_degree_list, degree, use_factoring;
        ...
        # morphism_4 := SEMISIMPLECATEGORY_Associator_4_Morphism( object_a, object_b, object_c, new_source, associator_data, is_magma_ring, is_complete_data );
        ...
        first_permutation := SEMISIMPLECATEGORY_Associator_123_Morphism( object_a, object_b, object_c, new_source, new_range );
        
        morphism_4 := SEMISIMPLECATEGORY_Associator_4_Morphism( object_a, object_b, object_c, new_source, associator_data, is_magma_ring, is_complete_data );
        
        second_permutation := SEMISIMPLECATEGORY_Associator_567_Morphism( object_a, object_b, object_c, new_source, new_range, use_factoring );
    ...
    AddCoevaluationForDualWithGivenTensorProduct( category,
      function( cat, unit, object, tensor_object )
        ...
        # Error( "\033[31mDEBUGPRINT[50]\033[0m" );
...
InstallMethod( SemisimpleCategory,
  ...
  function( homalg_field, membership_function, tensor_unit, associator_filename, is_complete_data, context_list )
    ...
    SetIsAdditiveMonoidalCategory( semisimple_category, true );
    ...
    SetAssociatorData( semisimple_category, associator_data );
