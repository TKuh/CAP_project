...
RepGMorphismToListOfPermutations :=
  function( mor_repg )
    ...
    Display( "Converting listlist's into matrices" );
    i := 1;
...
CompareBraidingMorphisms :=
  function( A, B )
    local timing_statistics, check_equality, AB, morphism_multiplicity, morphism, morphism_repg, matrices_repg, nr_support, morphism_multiplicity_sgreps, matrices, matrices_multiplicity, permutations_mor_sgreps, equal, equal2;
    ...
    if timing_statistics then
        StartTimer( "SGREPS_Braiding_12_Morphism_multiplicity" );
    fi;
    
    morphism_multiplicity := SGREPS_Braiding_12_Morphism_multiplicity( product_kron_comon, A, B, AB );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_12_Morphism_multiplicity" );
        DisplayTimer( "SGREPS_Braiding_12_Morphism_multiplicity" );
        ResetTimer( "SGREPS_Braiding_12_Morphism_multiplicity" );
        Print( "\n" );
        StartTimer( "SGREPS_Braiding_12_Morphism" );
    fi;
    
    morphism := SGREPS_Braiding_12_Morphism( sgreps,
                                             ConvertObjectToSGReps( A ),
                                             ConvertObjectToSGReps( B ),
                                             ConvertObjectToSGReps( AB ) );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_12_Morphism" );
        DisplayTimer( "SGREPS_Braiding_12_Morphism" );
        ResetTimer( "SGREPS_Braiding_12_Morphism" );
        Print( "\n" );
        StartTimer( "SEMISIMPLECATEGORY_Braiding_12_Morphism" );
    fi;
    
    morphism_repg := SEMISIMPLECATEGORY_Braiding_12_Morphism( ConvertObjectToRepG( A ),
                                                              ConvertObjectToRepG( B ) );
    
    if timing_statistics then
        StopTimer( "SEMISIMPLECATEGORY_Braiding_12_Morphism" );
        DisplayTimer( "SEMISIMPLECATEGORY_Braiding_12_Morphism" );
        ResetTimer( "SEMISIMPLECATEGORY_Braiding_12_Morphism" );
        Print( "\n" );
    fi;
    ...
    if check_equality then
        ...
        morphism_multiplicity_sgreps := EmbeddingProductCatOfPermutationCatIntoSGRepsOnMorphism(
                                            sgreps,
                                            morphism_multiplicity );
                                            # InverseForMorphisms( morphism_multiplicity ) );
        
        matrices_multiplicity := Components( morphism_multiplicity_sgreps );
...
CompareBraidingMorphisms( BC, B : timing_statistics := true, check_equality := true );
