...
CompareBraidingMorphisms :=
  function( A, B )
    ...
    if timing_statistics then
        StartTimer( "SGREPS_Braiding_1_Morphism_multiplicity" );
    fi;
    
    morphism_multiplicity := SGREPS_Braiding_1_Morphism_multiplicity( product_kron_comon, A, B, AB );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_1_Morphism_multiplicity" );
        DisplayTimer( "SGREPS_Braiding_1_Morphism_multiplicity" );
        ResetTimer( "SGREPS_Braiding_1_Morphism_multiplicity" );
        Print( "\n" );
        StartTimer( "SGREPS_Braiding_1_Morphism" );
    fi;
    
    morphism := SGREPS_Braiding_1_Morphism( sgreps,
                                             ConvertObjectToSGReps( A ),
                                             ConvertObjectToSGReps( B ),
                                             ConvertObjectToSGReps( AB ) );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_1_Morphism" );
        DisplayTimer( "SGREPS_Braiding_1_Morphism" );
        ResetTimer( "SGREPS_Braiding_1_Morphism" );
        Print( "\n" );
        StartTimer( "SEMISIMPLECATEGORY_Braiding_1_Morphism" );
    fi;
    
    morphism_repg := SEMISIMPLECATEGORY_Braiding_1_Morphism( ConvertObjectToRepG( A ),
                                                              ConvertObjectToRepG( B ) );
    
    if timing_statistics then
        StopTimer( "SEMISIMPLECATEGORY_Braiding_1_Morphism" );
        DisplayTimer( "SEMISIMPLECATEGORY_Braiding_1_Morphism" );
        ResetTimer( "SEMISIMPLECATEGORY_Braiding_1_Morphism" );
        Print( "\n" );
    fi;
