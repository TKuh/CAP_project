...
CompareRightDistributivityExpandings :=
  function( L, factor, multiplicities  )
    ...
    DisplayTimer( "RightDistExpandingWithMultiplicities" );
    ResetTimer( "RightDistExpandingWithMultiplicities" );
    Print( "\n" );
    
    StartTimer( "SGReps_RightDistExpanding" );
    mor_sgreps := RightDistributivityExpandingWithGivenObjects( sgreps, source_sgreps, L_sgreps, factor_sgreps, target_sgreps );
    StopTimer( "SGReps_RightDistExpanding" );
    DisplayTimer( "SGReps_RightDistExpanding" );
    ResetTimer( "SGReps_RightDistExpanding" );
    Print( "\n" );
    
    StartTimer( "SGReps_functional_RightDistExpanding" );
    mor_functional_sgreps := SGREPS_RightDistributivityExpandingPermutation( sgreps, L_sgreps, factor_sgreps, target_sgreps );
    ...
    StopTimer( "SGReps_functional_RightDistExpanding" );
    DisplayTimer( "SGReps_functional_RightDistExpanding" );
    ResetTimer( "SGReps_functional_RightDistExpanding" );
    Print( "\n" );
    
    StartTimer( "RepG_RightDistExpanding" );
    mor_repg := RightDistributivityExpandingWithGivenObjects( source_repg, L_repg, factor_repg, target_repg );
    StopTimer( "RepG_RightDistExpanding" );
    DisplayTimer( "RepG_RightDistExpanding" );
    ResetTimer( "RepG_RightDistExpanding" );
    Print( "\n" );
...
CompareMultiplicityToNonMultiplicity :=
  function( L, factor, multiplicities )
    local check_equality, L_expanded_with_multiplicities, source, target, mor_mults_sgreps, L_sgreps, factor_sgreps, source_sgreps, target_sgreps, mor_sgreps, mor_functional_sgreps;
...
multiplicities := [ 1, 1 ];;
factor := S3;;
CompareMultiplicityToNonMultiplicity( L, factor, multiplicities );;
...
multiplicities := [ 1, 1 ];;
factor := S3;;
CompareRightDistributivityExpandings( L, factor, multiplicities : check_equality := true );
# true
...
multiplicities := [ 60, 29, 105 ];;
factor := S4;;
CompareRightDistributivityExpandings( L, factor, multiplicities : check_equality := false );;
...
factor := A;;
CompareRightDistributivityExpandings( L, factor, multiplicities : check_equality := false );;
# Timer RightDistExpandingWithMultiplicities (stopped): started 
# 1 times with a total runtime of 44380 ms ( = 44380000 μs per run)
# 
# Timer SGReps_RightDistExpanding (stopped): started 
# 1 times with a total runtime of 44907 ms ( = 44907000 μs per run)
# 
# Timer SGReps_functional_RightDistExpanding (stopped): started 
# 1 times with a total runtime of 44283 ms ( = 44283000 μs per run)
# 
# Timer RepG_RightDistExpanding (stopped): started 
