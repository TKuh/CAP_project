...
CompareBraidingMorphisms :=
  function( A, B )
    local timing_statistics, check_equality, AB, morphism_multiplicity, morphism, morphism_repg, permutations_repg, nr_support, components, permutations_multiplicity, equal, equal2;
    ...
    if timing_statistics then
        StartTimer( "SGREPS_Braiding_2_Morphism_multiplicity" );
    fi;
    
    morphism_multiplicity := SGREPS_Braiding_2_Morphism_multiplicity( product_kron_comon, A, B, AB );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_2_Morphism_multiplicity" );
        DisplayTimer( "SGREPS_Braiding_2_Morphism_multiplicity" );
        ResetTimer( "SGREPS_Braiding_2_Morphism_multiplicity" );
        Print( "\n" );
        StartTimer( "SGREPS_Braiding_2_Morphism" );
    fi;
    
    morphism := SGREPS_Braiding_2_Morphism( sgreps,
                                             ConvertObjectToSGReps( A ),
                                             ConvertObjectToSGReps( B ),
                                             ConvertObjectToSGReps( AB ) );
    
    if timing_statistics then
        StopTimer( "SGREPS_Braiding_2_Morphism" );
        DisplayTimer( "SGREPS_Braiding_2_Morphism" );
        ResetTimer( "SGREPS_Braiding_2_Morphism" );
        Print( "\n" );
        StartTimer( "SEMISIMPLECATEGORY_Braiding_2_Morphism" );
    fi;
    
    permutations_repg := SEMISIMPLECATEGORY_Braiding_2_Morphism( ConvertObjectToRepG( A ),
                                                              ConvertObjectToRepG( B ) );
    
    if timing_statistics then
        StopTimer( "SEMISIMPLECATEGORY_Braiding_2_Morphism" );
        DisplayTimer( "SEMISIMPLECATEGORY_Braiding_2_Morphism" );
        ResetTimer( "SEMISIMPLECATEGORY_Braiding_2_Morphism" );
        Print( "\n" );
    fi;
    ...
    if check_equality then
        ...
        F := IsomorphismFromCoreToProductCategoryOfPermutationCategory( product_kron_comon );
        
        nr_support := NrSupport( morphism_multiplicity );
        
        permutations_multiplicity := Components( ApplyFunctor( F, morphism_multiplicity ) );
