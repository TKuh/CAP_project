...
CompareAssociatorMorphisms :=
  function( A, B, C )
    ...
    if timing_statistics then
        StartTimer( "SGREPS_Associator_7_Morphism_multiplicity" );
    fi;
    
    permutations_product_permcat := SGREPS_Associator_7_Morphism_multiplicity( product_kron_comon, A, B, C, ABC );
    
    if timing_statistics then
        StopTimer( "SGREPS_Associator_7_Morphism_multiplicity" );
        DisplayTimer( "SGREPS_Associator_7_Morphism_multiplicity" );
        ResetTimer( "SGREPS_Associator_7_Morphism_multiplicity" );
        Print( "\n" );
        StartTimer( "SGREPS_Associator_7_Morphism" );
    fi;
    
    permutations_sgreps := SGREPS_Associator_7_Morphism( sgreps,
    ...
    if timing_statistics then
        StopTimer( "SGREPS_Associator_7_Morphism" );
        DisplayTimer( "SGREPS_Associator_7_Morphism" );
        ResetTimer( "SGREPS_Associator_7_Morphism" );
        Print( "\n" );
        StartTimer( "SEMISIMPLECATEGORY_Associator_7_Morphism" );
    fi;
    
    use_factoring := true;
    permutations_repg := SEMISIMPLECATEGORY_Associator_7_Morphism( ConvertObjectToRepG( A ),
    ...
    if timing_statistics then
        StopTimer( "SEMISIMPLECATEGORY_Associator_7_Morphism" );
        DisplayTimer( "SEMISIMPLECATEGORY_Associator_7_Morphism" );
        ResetTimer( "SEMISIMPLECATEGORY_Associator_7_Morphism" );
        Print( "\n" );
    fi;
...
# Timer SGREPS_Associator_7_Morphism_multiplicity (stopped): started 
# 1 times with a total runtime of 189 ms ( = 189000 μs per run)
#
# Timer SGREPS_Associator_7_Morphism (stopped): started 
# 1 times with a total runtime of 18 ms ( = 18000 μs per run)
#
# Timer SEMISIMPLECATEGORY_Associator_7_Morphism (stopped): started 
# 1 times with a total runtime of 30 ms ( = 30000 μs per run)
#
# true

CompareAssociatorMorphisms( B, A, C : timing_statistics := true, check_equality := true );
# Timer SGREPS_Associator_7_Morphism_multiplicity (stopped): started 
# 1 times with a total runtime of 184 ms ( = 184000 μs per run)
#
# Timer SGREPS_Associator_7_Morphism (stopped): started 
# 1 times with a total runtime of 12 ms ( = 12000 μs per run)
#
# Timer SEMISIMPLECATEGORY_Associator_7_Morphism (stopped): started 
# 1 times with a total runtime of 18 ms ( = 18000 μs per run)
#
