...
InstallMethod( ADDITIVE_CLOSURE_Of_OBJECT_FINITE_CATEGORY,
  ...
  function( CAP_NAMED_ARGUMENTS, underlying_category )
    local AC_objfin, name, nr_objects_underlying_category, compare_morphisms, underlying_range_category,
          range_category, object_function, morphism_function, object_function_inverse, morphism_function_inverse;
    ...
    then
        ...
        SetCommutativeRingOfLinearCategory( AC_objfin, CommutativeRingOfLinearCategory( underlying_category ) );
    ...
    AddObjectConstructor( AC_objfin,
      function( AC_objfin, nr_summands_and_multiplicities )
        ...
        #% CAP_JIT_DROP_NEXT_STATEMENT
        if not Length( nr_summands_and_multiplicities[2] ) = nr_objects_underlying_category then
        ...
        #% CAP_JIT_DROP_NEXT_STATEMENT
        if ForAny( nr_summands_and_multiplicities[2], multiplicity ->
            not IsInt( multiplicity ) or multiplicity < 0 )
        then
    ...
    AddMorphismConstructor( AC_objfin,
      function( AC_objfin, source, morphism_matrix, target )
        ...
        #% CAP_JIT_DROP_NEXT_STATEMENT
        Assert( 0, IsList( morphism_matrix ) );
        
        #% CAP_JIT_DROP_NEXT_STATEMENT
        Assert( 0, Length( morphism_matrix ) = NrOfSummands( source ) );
        
        #% CAP_JIT_DROP_NEXT_STATEMENT
        Assert( 0, ForAll( morphism_matrix, row -> IsList( row ) and
                                                   Length( row ) = NrOfSummands( target ) ) );
    ...
    if CanCompute( underlying_category, "IsWellDefinedForObjects" ) then
        ...
        AddIsWellDefinedForObjects( AC_objfin,
          function( AC_objfin, object )
            ...
            if not IsList( nr_summands_and_multiplicities ) then
                
                return false;
                
            fi;

            if not Length( nr_summands_and_multiplicities ) = 2 then
                
                return false;
                
            fi;

            if not IsList( nr_summands_and_multiplicities[2] ) then
            ...
            if not Length( nr_summands_and_multiplicities[2] ) = NumberOfObjectsOfUnderlyingCategory( AC_objfin ) then
    ...
    if CanCompute( underlying_category, "IsWellDefinedForMorphismsWithGivenSourceAndRange" ) then
        ...
        AddIsWellDefinedForMorphisms( AC_objfin,
          function( AC_objfin, morphism )
            ...
            source_list := UnderlyingObjectList( Source( morphism ) );
            target_list := UnderlyingObjectList( Range( morphism ) );
            ...
            if not ( IsList( MorphismMatrix( morphism ) ) and Length( MorphismMatrix( morphism ) ) = nr_rows ) then
                ...
                return false;
                        ...
                        ForAll( [ 1 .. nr_cols ], j ->
                            IsWellDefinedForMorphismsWithGivenSourceAndRange( UnderlyingCategory( AC_objfin ),
                                                                              source_list[i],
                                                                              morphism[i, j],
                                                                              target_list[j] ) ) )
            then
    ...
    AddIdentityMorphism( AC_objfin,
      function( AC_objfin, object )
        ...
        object_list := UnderlyingObjectList( object );
    ...
    AddZeroMorphism( AC_objfin,
      function( AC_objfin, source, target )
        ...
        object_list_source := UnderlyingObjectList( source );
        
        object_list_target := UnderlyingObjectList( target );
    ...
    AddIsZeroForMorphisms( AC_objfin,
      function( AC_objfin, morphism )
        ...
        return ForAll( [ 1 .. NrRows( morphism ) ], i ->
                   ForAll( [ 1 .. NrCols( morphism ) ], j ->
    ...
    AddAdditionForMorphisms( AC_objfin,
      function( AC_objfin, morphism_1, morphism_2 )
        ...
        morphism_matrix :=
            List( [ 1 .. NrRows( morphism_1 ) ], i ->
                List( [ 1 .. NrCols( morphism_1 ) ], j ->
    ...
    AddSumOfMorphisms( AC_objfin,
      function( AC_objfin, source, morphisms, target )
        ...
        source_object_list := UnderlyingObjectList( source );
        target_object_list := UnderlyingObjectList( target );
    ...
    AddAdditiveInverseForMorphisms( AC_objfin,
      function( AC_objfin, morphism )
        ...
        morphism_matrix :=
            List( [ 1 .. NrRows( morphism ) ], i ->
                List( [ 1 .. NrCols( morphism ) ], j ->
    ...
    AddZeroObject( AC_objfin,
      function( AC_objfin )
        ...
        return AdditiveClosureObject( AC_objfin, [ 0, zero_list ] );
    ...
    if CanCompute( underlying_category, "MultiplyWithElementOfCommutativeRingForMorphisms" ) then
      ...
      AddMultiplyWithElementOfCommutativeRingForMorphisms( AC_objfin,
        function( AC_objfin, r, alpha )
          ...
          morphism_matrix :=
              List( [ 1 .. NrRows( alpha ) ], i ->
                  List( [ 1 .. NrCols( alpha ) ], j ->
            ...
            AddHomomorphismStructureOnObjects( AC_objfin,
              function( cat, object_1, object_2 )
                ...
                return DirectSum( range_category,
                          List( [ 1 .. Length( UnderlyingObjectList( object_1 ) ) ], j ->
                            DirectSum( range_category,
                              List( [ 1 .. Length( UnderlyingObjectList( object_2 ) ) ], s ->
                                HomomorphismStructureOnObjectsExtendedByFullEmbedding(
                                    UnderlyingCategory( cat ),
                                    range_category,
                                    UnderlyingObjectList( object_1 )[j],
                                    UnderlyingObjectList( object_2 )[s] ) ) ) ) );
           ...
           (ForAll( [ "PreCompose",
            ...
            AddInterpretMorphismFromDistinguishedObjectToHomomorphismStructureAsMorphism( AC_objfin,
              function( cat, B, C, morphism )
                ...
                size_j := Length( UnderlyingObjectList( B ) );
                
                size_s := Length( UnderlyingObjectList( C ) );
        ...
        if not ( HasIsStrictMonoidalCategory( underlying_category ) and IsStrictMonoidalCategory( underlying_category ) ) then
            ...
            AddRightUnitorWithGivenTensorProduct( AC_objfin,
              function( AC_objfin, obj, obj_tensor_unit )
                ...
                length := Length( UnderlyingObjectList( obj ) );
            ...
            AddAssociatorLeftToRightWithGivenTensorProducts( AC_objfin,
              function( AC_objfin, source, obj_1, obj_2, obj_3, target )
                ...
                len_1 := Length( UnderlyingObjectList( obj_1 ) );
                len_2 := Length( UnderlyingObjectList( obj_2 ) );
                len_3 := Length( UnderlyingObjectList( obj_3 ) );
    ...
    # HandlePrecompiledTowers( category, underlying_category, "AdditiveClosureOfObjectFiniteCategory" );
...
InstallMethodForCompilerForCAP( \[\,\],
  ...
  function( morphism, i, j )
    ...
    if not ( 0 < i and i <= NrRows( morphism ) and 0 < j and j <= NrCols( morphism ) ) then
