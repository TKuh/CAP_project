...
#! Let $C$ be a pre-additive category and $C^\oplus$ be its additive closure.
...
#! can be achieved by only remembering the number of occurences of each $o_j$ in the object list of $o$.
#! 
#! For example: assume $C$ contains exactly four objects $\{ o_1, o_2, o_3, o_4 \}$ and let
#! @BeginLatexOnly
#! \begin{center}
...
#! Here, the object $o_1$ occurs one time, $o_2$ occurs three times, $o_3$ occurs two times
...
#! the morphisms are also given by such matrices, however, the ordering of the
#! underlying objects has to be respected.
#! 
#! For example, a morphism $o_1 \oplus o_1 \oplus o_2 \rightarrow o_2 \oplus o_3$
...
CapJitAddTypeSignature( "AdditiveClosureObject", [ IsAdditiveClosureOfObjectFiniteCategory, IsList ], function ( input_types )
...
DeclareAttribute( "UnderlyingObjectList", IsObjectInAdditiveClosureOfObjectFiniteCategory );

CapJitAddTypeSignature( "UnderlyingObjectList", [ IsObjectInAdditiveClosureOfObjectFiniteCategory ],
  function ( input_types )
...
#! @Arguments alpha
#! @Returns an integer
DeclareAttribute( "NrRows", IsMorphismInAdditiveClosureOfObjectFiniteCategory );

CapJitAddTypeSignature( "NrRows", [ IsMorphismInAdditiveClosureOfObjectFiniteCategory ], IsBigInt );
...
CapJitAddTypeSignature( "NrColumns", [ IsMorphismInAdditiveClosureOfObjectFiniteCategory ], IsBigInt );
